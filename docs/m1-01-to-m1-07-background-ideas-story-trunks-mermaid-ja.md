...
```mermaid
flowchart LR
    A1["交通障害・配送需要・<br/>電力制約が変化する"]
    A2["複数事業者の配送資源と<br/>エネルギー資源を把握する"]
    A3["車両・経路・倉庫・受取拠点・<br/>充電枠・電力容量を同時に再計画する"]
    A4["運用担当者が複数案を<br/>確認・修正して実行する"]
    A5["配送を継続し、充電待ちや<br/>受取側の再調整を減らす"]

    A1 --> A2 --> A3 --> A4 --> A5

    B1["交通・配送需要・<br/>電力使用の予測"]
    B2["車両の電池残量・<br/>消費量の推定"]
    B3["配送会社を越えた<br/>車両・拠点の利用"]
    B4["配送計画と充電計画の<br/>同時最適化"]
    B5["障害発生時の<br/>リアルタイム再計画"]
    B6["上流の電池・材料研究との接続"]
    B7["古典計算・AI・量子計算の<br/>役割分担"]
    B8["配送員・生活者・事業者に<br/>生じる変化"]
...
```mermaid
flowchart LR
    C1["予定・食事履歴・<br/>家庭内在庫が変化する"]
    C2["現在の食事状態と<br/>利用可能な食品を把握・推定する"]
    C3["栄養・好み・価格・旬・<br/>調理時間・商品在庫を同時に比較する"]
    C4["複数の食事候補と<br/>購入・受取候補を生成する"]
    C5["生活者が候補や条件を<br/>確認・修正して選ぶ"]
    C6["食事記録・献立検討・買物を<br/>個別に管理する負担を減らす"]

    C1 --> C2 --> C3 --> C4 --> C5 --> C6

    D1["食事内容の自動取得・状態推定"]
    D2["家庭内在庫・保存状態の把握"]
    D3["栄養・価格・好み・時間の<br/>多条件比較"]
    D4["店舗の商品在庫との接続"]
    D5["食品安全・腐敗・真正性の確認"]
    D6["食品ロス削減への波及"]
    D7["古典計算・AI・量子計算の<br/>役割分担"]
    D8["生活者・小売・食品供給側に<br/>生じる変化"]
...
```mermaid
flowchart LR
    S2["Story 2<br/>食事候補を選ぶ"]
    O["必要な食品・数量・<br/>受取条件が決まる"]
    S1["Story 1<br/>配送・充電・電力制約を含む<br/>配送計画へ渡す"]
