...
def validate_yaml() -> None:
    for directory in ("content", "data/public", "schemas"):
...
def validate_derivation_maps() -> None:
    schema = json.loads(
        (ROOT / "schemas/derivation-map.schema.json").read_text(encoding="utf-8")
    )
    validator = Draft202012Validator(schema)

    for path in (ROOT / "content/use-cases").rglob("engineering-map-2055.yml"):
...
def validate_target_year() -> None:
    data = load_yaml(ROOT / "content/framework/target-year.yml")
